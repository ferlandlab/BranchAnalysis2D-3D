/*
 * #%L
 * AnalyzeSkeletonThresholding_ plugin for ImageJ.
 * %%
...
/**
 * AnalyzeSkeletonThresholding_ plugin for ImageJ and Fiji.
 *
 ...
 *
 */
public class MorphometryBranchAnalysis_ implements PlugInFilter, DialogListener
{
	...
	private static final int DEFAULT_PRUNE_MODE_INDEX = MorphometryBranchAnalysis_.NONE;
	private static final boolean DEFAULT_PRUNE_ENDS = false;
	private static final boolean DEFAULT_CALCULATE_SHORTEST_PATH = true;
	...
	public static byte SPINE_PATH = 96;

	private GenericDialog settingsDialog;

	/** working image plus */
	private ImagePlus imRefThresholded = null;
	private ImagePlus imRefSkeleton = null;
	private ImagePlus imRefThickness = null;
	...
	private ImageStack spinePathImage = null;
	...
	public int setup(String arg, ImagePlus imp) 
	{
		...
		this.imRefThresholded = imp;

		// we are going to be skeletonizing the thresholded image provided 
		// as input before doing the branching
		{
			this.imRefSkeleton = this.imRefThresholded.duplicate();
			this.imRefSkeleton.setTitle(imRefSkeleton.getTitle().replace("DUP","SKEL"));

			Binary binary = new Binary();
			binary.setup("skel", this.imRefSkeleton);
			ImageStack stack = this.imRefSkeleton.getStack();
			for (int i = 1; i <= this.imRefSkeleton.getStack().getSize(); i++)
			{
				ImageProcessor skeletonIp = stack.getProcessor(i);
				skeletonIp.setPixels(stack.getPixels(i));
				skeletonIp.setSliceNumber(i);
				binary.run(skeletonIp);
				stack.setPixels(skeletonIp.getPixels(), i);
			}
			this.imRefSkeleton.updateAndDraw();
		}
	
 		// thickness computation on original images
		//Local_Thickness_Driver thickness = new Local_Thickness_Driver();
		{
			EDT_S1D thickness = new EDT_S1D();
			thickness.runSilent = true;
			thickness.setup(null, this.imRefThresholded);
			ImageStack stack = this.imRefThresholded.getStack();
			for (int i = 1; i <= this.imRefThresholded.getStack().getSize(); i++)
			{
				ImageProcessor thicknessIp = stack.getProcessor(i);
				thicknessIp.setPixels(stack.getPixels(i));
				thicknessIp.setSliceNumber(i);
				thickness.run(thicknessIp);
				//stack.setPixels(thicknessIp.getPixels(), i);
			}
			this.imRefThickness = thickness.getResultImage();
			this.imRefThickness.show();
			IJ.run("Fire");
		}
	...
	public void run(ImageProcessor ip) 
	{
		...
		{
			// No pruning
			case MorphometryBranchAnalysis_.NONE:
				this.bPruneCycles = false;
				break;
			// Pruning cycles by shortest branch
			case MorphometryBranchAnalysis_.SHORTEST_BRANCH:
				this.bPruneCycles = true;
				break;
			// Pruning cycles by lowest pixel intensity
			case MorphometryBranchAnalysis_.LOWEST_INTENSITY_VOXEL:
			case MorphometryBranchAnalysis_.LOWEST_INTENSITY_BRANCH:
		...
		run(pruneIndex, pruneEnds, calculateShortestPath, origIP, false,
				verbose, (protectRoi) ? this.imRefSkeleton.getRoi() : null);

		if(debug)
			IJ.log("num of skeletons = " + this.numOfTrees);

		// Show labeled skeletons
		if( MorphometryBranchAnalysis_.displaySkeletons )
		{
			ImagePlus labeledSkeletons = 
					new ImagePlus( this.imRefSkeleton.getShortTitle() 
	...
	{
/*		if (this.skeletonImRef.getRoi()==null)
		{
...
*/		return true;
	...
	public SkeletonResult run(
			...
			Roi roi)
	{
		MorphometryBranchAnalysis_.pruneIndex = pruneIndex;
		this.silent = silent;
		MorphometryBranchAnalysis_.pruneEnds = pruneEnds;
		MorphometryBranchAnalysis_.calculateShortestPath = shortPath;
		MorphometryBranchAnalysis_.verbose = verbose;
		...
		this.width = this.imRefSkeleton.getWidth();
		this.height = this.imRefSkeleton.getHeight();
		this.depth = this.imRefSkeleton.getStackSize();
		this.inputImage = this.imRefSkeleton.getStack();
		...
		if(bPruneCycles)
		{
			if(pruneCycles(this.inputImage, this.originalImage, MorphometryBranchAnalysis_.pruneIndex))
			{
				// initialize visit flags
				resetVisited();
		...
		if(shortPath)
		{
			...
			this.spinePathImage = new ImageStack(this.width, this.height, this.inputImage.getColorModel());
			for(int i=1; i<=this.inputImage.getSize(); i++)
				spinePathImage.addSlice(this.inputImage.getSliceLabel(i), this.inputImage.getProcessor(i).duplicate());
			...
			for(int i = 0; i < this.numOfTrees; i++)
			{
				...
				spStartPosition[i][0] = spx * this.imRefSkeleton.getCalibration().pixelWidth;
				spStartPosition[i][1] = spy * this.imRefSkeleton.getCalibration().pixelHeight;
				spStartPosition[i][2] = spz * this.imRefSkeleton.getCalibration().pixelDepth;
			}
			...
			if (!silent) {
				...
				shortIP.setCalibration(this.imRefSkeleton.getCalibration());
			...
			boolean verbose)
	{
		MorphometryBranchAnalysis_.pruneIndex = pruneIndex;
		this.silent = silent;
		MorphometryBranchAnalysis_.pruneEnds = true;
		MorphometryBranchAnalysis_.calculateShortestPath = shortPath;
		MorphometryBranchAnalysis_.verbose = verbose;
	 ...
	 * or showing images.
	 * <p>
	 * This one just calls run(AnalyzeSkeletonThresholding_.NONE, false, null, true, false)
	 */
	public SkeletonResult run()
	{
				...
				if(backEdges.size() > 0)
				{
					...
					for(final Edge e : backEdges)
					{
						...
						while(!finalLoopVertex.equals(backtrackVertex))
						{
							...
							if(pruningMode == MorphometryBranchAnalysis_.SHORTEST_BRANCH && 
						...
						else if (pruningMode == MorphometryBranchAnalysis_.LOWEST_INTENSITY_VOXEL)
						{
							removeLowestIntensityVoxel(loopEdges, inputImage, originalImage);
						}
						else if(pruningMode == MorphometryBranchAnalysis_.LOWEST_INTENSITY_BRANCH)
						{
	...
	{
		final int slices = imRefSkeleton.getNSlices();
		final int frames = imRefSkeleton.getNFrames();
		final int channels = imRefSkeleton.getNChannels();

		ImagePlus tagIP = IJ.createHyperStack("Tagged skeleton", width, height, channels, slices, frames,
											  imRefSkeleton.getBitDepth());
		tagIP.setStack(taggedImage.duplicate(), channels, slices, frames);
		tagIP.show();

		// Set same calibration as the input image
		tagIP.setCalibration(this.imRefSkeleton.getCalibration());
	...
	private void showResults() 
	{
		...
		// Extra information
		if(MorphometryBranchAnalysis_.verbose)
		{
			// New results table
			...
			final String[] extra_head = {"Branch", "Skeleton ID", 
							...
							"max thickness","average thickness",
							"spine",
			...
			for(int i = 0 ; i < this.numOfTrees; i++)
			{
				...
				for(final Edge e : listEdges)
				{
					...
					extra_rt.addValue(extra_head[3], e.getV1().getPoints().get(0).x * this.imRefSkeleton.getCalibration().pixelWidth);
					extra_rt.addValue(extra_head[4], e.getV1().getPoints().get(0).y * this.imRefSkeleton.getCalibration().pixelHeight);
					extra_rt.addValue(extra_head[5], e.getV1().getPoints().get(0).z * this.imRefSkeleton.getCalibration().pixelDepth);
					extra_rt.addValue(extra_head[6], e.getV2().getPoints().get(0).x * this.imRefSkeleton.getCalibration().pixelWidth);
					extra_rt.addValue(extra_head[7], e.getV2().getPoints().get(0).y * this.imRefSkeleton.getCalibration().pixelHeight);
					extra_rt.addValue(extra_head[8], e.getV2().getPoints().get(0).z * this.imRefSkeleton.getCalibration().pixelDepth);
					...
					// MODIFIED
					extra_rt.addValue(extra_head[11], e.getMaxThickness() );//* this.imRefSkeleton.getCalibration().pixelWidth);
					extra_rt.addValue(extra_head[12], e.getAvgThickness() );//* this.imRefSkeleton.getCalibration().pixelWidth);

					if (shortestPathPoints[i].contains(e.getV1().getPoints().get(0))
							&& shortestPathPoints[i].contains(e.getV2().getPoints().get(0)))
					{
						extra_rt.addValue(extra_head[13],0);
					} else {
						extra_rt.addValue(extra_head[13],1);
						for (Point p : e.getSlabs())
							setPixel(this.spinePathImage, p.x, p.y, p.z, SPINE_PATH);
						for (Point p : e.getV1().getPoints())
							setPixel(this.spinePathImage, p.x, p.y, p.z, SPINE_PATH);
						for (Point p : e.getV2().getPoints())
							setPixel(this.spinePathImage, p.x, p.y, p.z, SPINE_PATH);
					}
					extra_rt.addValue(extra_head[14], e.getColor3rd());
					extra_rt.addValue(extra_head[15], e.getColor());
				}

			}
			{
				ImagePlus spineIP = new ImagePlus("Spine paths", spinePathImage);
				spineIP.show();
				spineIP.setCalibration(this.imRefSkeleton.getCalibration());
				IJ.run(spineIP, "Fire", null);
				spineIP.resetDisplayRange();
				spineIP.updateAndDraw();
	...
	void computeThickness(Edge edge)
	{
		int numPoints = 2 + edge.getSlabs().size();
		{
			//int[] v = thicknessImRef.getPixel(edge.getV1().getPoints().get(0).x, edge.getV1().getPoints().get(0).y);
			//double value = Float.intBitsToFloat(v[0]);
			double value = imRefThickness.getStack().getVoxel(
									edge.getV1().getPoints().get(0).x, 
									edge.getV1().getPoints().get(0).y,
									edge.getV1().getPoints().get(0).z);
			if (value > edge.getMaxThickness())
				edge.setMaxThickness(value);
			edge.setAvgThickness(edge.getAvgThickness() + value);
		}
		{
			//int[] v = thicknessImRef.getPixel(edge.getV2().getPoints().get(0).x, edge.getV2().getPoints().get(0).y);
			//double value = Float.intBitsToFloat(v[0]);
			double value = imRefThickness.getStack().getVoxel(
					edge.getV2().getPoints().get(0).x, 
					edge.getV2().getPoints().get(0).y,
					edge.getV2().getPoints().get(0).z);
			if (value > edge.getMaxThickness())
				edge.setMaxThickness(value);
			edge.setAvgThickness(edge.getAvgThickness() + value);
		}
		{
			...
			{
				//int[] v = thicknessImRef.getPixel(p.x, p.y);
				//double value = Float.intBitsToFloat(v[0]);
				double value = imRefThickness.getStack().getVoxel(p.x,p.y,p.z);
				if (value > edge.getMaxThickness())
					edge.setMaxThickness(value);
				edge.setAvgThickness(edge.getAvgThickness() + value);
		...
		edge.setAvgThickness(edge.getAvgThickness()/numPoints);
	}
	...
	private ImageStack markTrees(ImageStack taggedImage) 
	{
		...
		if(debug)
		{
			...
			// Set same calibration as the input image
			treesIP.setCalibration(this.imRefSkeleton.getCalibration());
	...
	{
		return Math.sqrt(  Math.pow( (point1.x - point2.x) * this.imRefSkeleton.getCalibration().pixelWidth, 2) 
				          + Math.pow( (point1.y - point2.y) * this.imRefSkeleton.getCalibration().pixelHeight, 2)
				          + Math.pow( (point1.z - point2.z) * this.imRefSkeleton.getCalibration().pixelDepth, 2));
	}
	...
	private double calculateDistance(List<Point> Points)
	{
		...
		return Math.sqrt(  Math.pow( (Points.get(poi).x - Points.get(0).x) * this.imRefSkeleton.getCalibration().pixelWidth, 2) 
		          + Math.pow( (Points.get(poi).y - Points.get(0).y) * this.imRefSkeleton.getCalibration().pixelHeight, 2)
		          + Math.pow( (Points.get(poi).z - Points.get(0).z) * this.imRefSkeleton.getCalibration().pixelDepth, 2))/poi;
	}
		...
		for (int iTree = 0; iTree < this.numOfTrees; iTree++)
		{
			...
			for(int i = 0; i < this.numberOfJunctions[iTree]; i ++)
			{
				...
				for(int j = 0; j < groupOfJunctions.size(); j++)
				{
					...
					for(int k = 0; k < 27; k++)
						if (neighborhood[k] == MorphometryBranchAnalysis_.SLAB 
							|| neighborhood[k] == MorphometryBranchAnalysis_.END_POINT)
							nBranch++;
				}
	...
	{
		return getPixel(this.taggedImage, point.x, point.y, point.z) == MorphometryBranchAnalysis_.SLAB;
	}
	...
	{
		return getPixel(this.taggedImage, point.x, point.y, point.z) == MorphometryBranchAnalysis_.JUNCTION;
	}
	...
	{
		return getPixel(this.taggedImage, point.x, point.y, point.z) == MorphometryBranchAnalysis_.END_POINT;
	}
	...
	{
		return getPixel(this.taggedImage, x, y, z) == MorphometryBranchAnalysis_.JUNCTION;
	}
			...
			for (int x = 0; x < width; x++) 
				for (int y = 0; y < height; y++)
				{
					if(getPixel(inputImage2, x, y, z) != 0)
					{
						...
						if(numOfNeighbors < 2)
						{
							setPixel(outputImage, x, y, z, MorphometryBranchAnalysis_.END_POINT);
							this.totalNumberOfEndPoints++;
						...
						{
							setPixel(outputImage, x, y, z, MorphometryBranchAnalysis_.JUNCTION);
						...
						{
							setPixel(outputImage, x, y, z, MorphometryBranchAnalysis_.SLAB);
	...
	private static final String EDT_THRESH_KEY = "edtS1.thresh";
	private static final String EDT_INVERSE_KEY = "edtS1.inverse";
	...
	private void createSettingsDialog() {
		...
		settingsDialog.addChoice("Prune cycle method: ", MorphometryBranchAnalysis_.pruneCyclesModes,
				MorphometryBranchAnalysis_.pruneCyclesModes[pruneIndex]);
		...
		settingsDialog.setInsets(20, 0, 0); //default top inset for subsequent checkboxes is 0
		settingsDialog.addMessage("EDT...", headerFont);
		settingsDialog.addNumericField("Threshold (1 to 255; value < thresh is background)",thresh, 0);
		settingsDialog.addCheckbox("Inverse case (background when value >= thresh)", inverse);
	...
	private void loadDialogSettings() {
		...
		thresh = (int) Prefs.get(EDT_THRESH_KEY, DEFAULT_THRESHOLD);
		inverse = Prefs.get(EDT_INVERSE_KEY, DEFAULT_INVERSE);
	}
	...
	private void saveDialogSettings() {
		...
		Prefs.set(EDT_THRESH_KEY, thresh);
		Prefs.set(EDT_INVERSE_KEY, inverse);
	...
	private void setSettingsFromDialog() {
		...
		thresh = (int) settingsDialog.getNextNumber();
		inverse = settingsDialog.getNextBoolean();
	}
}// end class AnalyzeSkeletonThresholding_
